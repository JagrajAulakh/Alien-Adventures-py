...
# Makes the window centered to screen
...
# Sets window icon
display.set_icon(transform.scale(image.load('images/icon.png'), (256,256)))		# Window Icon
...
# Converts a string to multiple lines depending on the max width, font, and font size
#		  Font path, text, size, width
...
# Cuts a sprite off a spritesheet and returns it
...
# Gets spritesheet coordinates from .txt file
...
# Makes every pixel on a Surface into a desired color
...
# Darkens a Surface to desired alpha value
...
# Makes a Surface translucent to desired alpha value
...
# Does the scrolling in-game
off_screen = False
def player_scroll():
	global all_sprites, player, off_screen
	go_ahead = False

	# Checks if the player is not on the screen
	if player.rect.left > width - width//5 or player.rect.right < width//5:
		off_screen = True

	# and then brings him onto the screen by scrolling by the difference 
	# between the center of the screen and the player's position
	if off_screen:
	...
	# If scrolling requirements are met above, scroll all the sprites by player velocity
	if go_ahead:
...
# Makes a level with a picture that stores all level objects
def level_creator(pic,player_type,background=0):
	...
	# Preserve the amount of lives the player previously had
	...
	# cycles through every pixel in the level image
	for x in range(p_width):
		for y in range(p_height):
			...
			# Determines the position of the object based on the pixel location in the image.
			# The alpha value of the color determins how much the object moves down by. 
			# E.g. and alpha value of 254 means to move the object down one block from where the actual pixel is
			pos = (player.size*16*x, height - p_height*player.size*16 + player.size*16*y + (255-c.a)*player.size*16)

			# Removes the alpha value in the color. The alpha value has been used above so, there's no need for it.
			c = c[:3]

			if c == (200, 200, 200):
				player.pos = vec(pos[0], pos[1])
				# Stores the spawn point of the current level
			...
			# Back block that is revealed when player touches it
			...
			# Back block that is always revealed
			...
			# Makes a key that only unlockes the closest locked block of the same color
			elif c in close_key_colors:
				tmp = Key(pos, close_key_colors.index(c), find_closest=True)

			# Makes a key that unlockes all locked blocks of the same color
	...
	# If the bridge is inside a hidden area, the should be put in the back_front_sprites group.
	# Otherwise, add it to the front_sprites group. The player doesn't see the back_front_sprites
	# until they enter a hidden area
...
# This fuction loads all the spritesheets and coordinates for the sheets
def load_images():
	...
	# Lets the user see the loading animation
	time.wait(1000)
...
# Menu scrolling: uses sine function to give smooth effect
	...
	for i in range(91):
		...
		if background:
			if d == 'right':
				...
				menu_back_rect.x += round(2.3 * cos(radians(i)))	# Black Magic
			elif d == 'up':
		...
		# Animates preview player in the menu
		current_animation = preview_animations[current_preview_player][int(ani_counter)%2]

		# The bubble around the preview player uses a sine function as well.
		tmp = round(4*sin(radians(ani_counter*70)))
		# Draws translucent bubble around preview player
		...
		# Adds a translucent background to the controls image
...
# Animation that blacks out the screen
...
# Animtion that draws a circle getting smaller and smaller.
...
# Creates a popup when the user has completed a level (used a door).
def level_complete(screen_copy, title='Level Complete!', transition_time=60, main_background_color=LIGHT_BROWN, title_background_color=DARK_BROWN):
	...
	# Surface for all buttons and pictures to lit on
	...
	# Blit everything onto the popup_surf for the scaling in
	...
	a = 0	# Counter for the darkness of the popup overlay
	for i in range(0, 121, 5):		# Transition for the popup scaling up
		a += 5
		screen.fill(WHITE)
		alpha_screen.fill((0,0,0,a))	# Darkens surroundings
		screen.blit(screen_copy, (0,0))		# What is behind the popup...

		coeff = popup_rect.width / sin(radians(121))							#
		new_x = coeff * sin(radians(i))											# Black magic
		new_y = coeff*popup_rect.height/popup_rect.width * sin(radians(i))		#
		scaled_popup_surf = transform.scale(popup_surf, (round(new_x), round(new_y)))	# Enlarge popup
		scaled_popup_rect = scaled_popup_surf.get_rect()
		scaled_popup_rect.center = width//2, height//2			# Align the scaling popup to center
		alpha_screen.blit(scaled_popup_surf, scaled_popup_rect)

		screen.blit(alpha_screen, (0,0))	# Overlay to popup
		display.flip()
		myClock.tick(FPS)

	outcome = None
	while outcome == None:	# Keeps running until a button is clicked.
		let_go = False
...
# Similar to the level_complete, but more customizable: Custom button text, custom title, custom header text...
def popup(title, header_txt, button_text, screen_copy, transition_time=60, main_background_color=LIGHT_BROWN, title_background_color=DARK_BROWN, main_button_color=(0,200,0), pics=[]):
	...
	a = 0	# Counter for the darkness of the popup overlay
	for i in range(0, 121, 5):		# Transition for the popup to be displayed to the screen
		a += 5
		screen.fill(WHITE)
		alpha_screen.fill((0,0,0,a))
		screen.blit(screen_copy, (0,0))		# What is behind the popup...
	...
	# Runs until the user clicks one of the popup buttons
	outcome = None
	pop_out = False
	while outcome == None:
		let_go = False
	...
	if pop_out:
		for i in range(120, -1, -5):		# Transition for the popup to be displayed to the screen
			a -= 5
			screen.fill(WHITE)
			alpha_screen.fill((0,0,0,a))
			screen.blit(screen_copy, (0,0))		# What is behind the popup...
...
# Pauses the game. Side bar appears and user can either click resume, restart or exit
def pause(s_copy):
...
# Looks through every pixel in a level image and counts the gem colours
...
# Give the level number and total amount of gems in that level, and this 
# function makes an image displaying the correct amount of gems collected.
def make_gem_image(level_num, gem_num):
	...
	for i in range(gem_num):
		if all_gems_collected[level_num][i]:		# If the user actually collected the gem
			img = actual_gem_images[i]		# get the appropriate images
		else:
			img = get_image(misc_sheet, misc_cos[25])	# get the blank gem image
...
# Reads all settings and game data from 'data.txt'
def read_data():
...
# Writes all settings and game data to 'data.txt'
def write_data():
	with open('data.txt', 'w') as f:
		f.write('music=%i\ncurrent theme=%i\nlevels unlocked=%i\n' % (int(music_bool), themes.index(current_theme), levels_unlocked))
		for i in all_gems_collected:
			# Found out this method of string formatting from a friend: Syed Safwaan
			f.write('{0} {1} {2} {3}\n'.format(*i))
		f.close()

# Button class makes a button that changes colour when hovering over it. Custom text, colors, pictures...
class Button:
	...
	def make_image(self, new_button_col, new_text_col):
		...
		# Either the pic or text. Can't have both
	...
	# Checks what every button should do when clicked on
...
# Class makes it much easier to create customizable characters. Just need to pass in the player type (0,1,2).
class Player(sprite.Sprite):
	def __init__(self, player_type=0, pos=[width//2,height//2]):
		...
		# Animations for the current player type
		...
		# Booleans for the state of the player
		self.jumping = True
		self.ducking = False
		self.in_water = False
	...
	# Moves the player. Includes key strokes and collisions with objects.
	def move(self):
